...
/**
 * Given many time intervals, find all conflict pairs. Design input/output and your own data structure if necessary.
 * 
 */
class Interval {
	int start;
...
class Pair<T> {
	...
	@Override
	public String toString() {
		return "[ first = " + interval.toString() + ", second = "
				+ conflicts.toString() + ']';
	}
}
...
public class AllConflictTimeIntervals {
	public List<Pair<Interval>> findAllConflictIntervals(
		...
		for (int i = 0; i < allIntervals.size() - 1; i++) {
			for (int j = allIntervals.size() - 1; j > i; j--) {
				if (allIntervals.get(j).getStart() < allIntervals.get(i)
						.getEnd()) {
					res.add(new Pair<>(allIntervals.get(i),
							allIntervals.subList(i + 1, j + 1)));
					break;
				}
			}
		}
